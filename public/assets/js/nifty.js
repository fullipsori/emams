  ...
  el.addEventListener("click", (e) => {
    ...
    if (el.getAttribute("data-view") == "fullscreen") {
      if (el.classList.contains("active")) {
        ...
        else if (el.targetEl.webkitRequestFullscreen)
          el.targetEl.webkitRequestFullscreen(); /* Safari */
      } else {
        /* Close fullscreen */
...
document
  ...
  .forEach((el) =>
    el.addEventListener("click", (e) => el.closest(".card").remove())
  );
...
(function () {
  ...
  miniNavContentsCollapse = miniNavContents.map((collapseEl) => {
    const activeToggler = collapseEl.parentElement.querySelector(
...
const addVariables = (el) => {
  const miniNavContentTarget =
    el.parentElement.querySelector(".mininav-content");
...
const hideAllMiniNavContent = (e) => {
  if (
    ...
    miniNavContentsCollapse.map((el) => el.hide());
};
...
const toggleContent = (e) => {
  ...
  ["click", "touchend"].forEach((evt) =>
    document.addEventListener(evt, hideAllMiniNavContent, { once: true })
  );
};
...
const buildNav = () => {
  ...
  if (!isMiniNav)
    activeSub = mainNav.querySelector(
  ...
  miniNavTogglers.map((miniNavToggler) => {
    ...
    if (!isMiniNav || window.innerWidth < 992) {
      ...
      ["mouseenter", "touchend"].forEach((evt) =>
        miniNavToggler.removeEventListener(evt, toggleContent)
      );
      ...
      miniNavToggler._mainnav.target.addEventListener(
        "hide.bs.collapse",
        bsCollapseHide
      );
    } else {
      ...
      miniNavToggler._mainnav.target.popper = Popper.createPopper(
        miniNavToggler,
        miniNavToggler._mainnav.target,
        popperOptions
      );
      miniNavToggler.removeEventListener("click", toggleContent);
      ["mouseenter", "touchend"].forEach((evt) =>
        miniNavToggler.addEventListener(evt, toggleContent)
      );

      // Hide all submenus when clicked outside of the main menu.
      miniNavToggler._mainnav.target.removeEventListener(
        "hide.bs.collapse",
        bsCollapseHide
      );
    }
